...
- Claude Code（最新版）
- OpenAI Codex CLI（用你手邊現有的版本即可，不鎖版本）
- Python 3.9+
- macOS / Linux / Windows（原生 PowerShell 或 WSL）

> **注意：** Windows 支援為 experimental（實驗性）。

## 運行護欄（Operational Safeguards）

Wrapper 內建的防呆機制，源自實際 dispatch 過程中遇到的 failure mode：

- **Watchdog**——當 wall-clock 超過 `--timeout-sec`（預設 1800 秒），或 stdout 靜默超過 `--heartbeat-sec`（預設 300 秒）時，直接 kill Codex subprocess。閒置 hang（疑似 ChatGPT stream 卡死）過去會默默燒掉 token 額度；wrapper 現在會 fail fast，並在 `policy.json` 記錄 `watchdog_killed: true`。
- **Quota gate**——當 Codex 5 小時 primary window `used_percent` 達到或超過 `--quota-gate`（預設 85）時拒絕派工。讀取 `~/.codex/auth.json`、查詢 codexbar 也在用的同一個 usage endpoint。`--force` 可覆寫；網路或 auth 出錯時 graceful skip。
- **Streaming events**——`events.jsonl` 在執行過程中即時逐行寫入，可 `tail -f` 活著的 dispatch，不必等 subprocess 退出。
...
| `--timeout-sec <n>` | Wall-clock 硬性 timeout，秒數（預設 1800） |
| `--heartbeat-sec <n>` | stdout 靜默超過此秒數即 kill（預設 300） |
| `--quota-gate <pct>` | 5 小時 Codex usage ≥ 此百分比時拒絕派工（預設 85） |
| `--force` | 覆寫 quota gate |
...
| `$CODEX_DISPATCH_TIMEOUT_SEC` | `--timeout-sec` 的預設值 |
| `$CODEX_DISPATCH_HEARTBEAT_SEC` | `--heartbeat-sec` 的預設值 |
